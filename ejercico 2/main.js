...
Myfrom.addEventListener("submit", function(e){
    ...
    document.querySelector(".containerP").innerText = "Los datos del formulario an sido registrados correctamente";
    dialogo.show();
    console.log(from);
    e.preventDefault();
})
...
let ListarDatos = async function(){
    ...
    for (let i = 0; i < localStorage.length; i++) {
        ...
        tr.insertAdjacentHTML("beforeend", "<button data-btn='Eliminar'>Eliminar</button>");
...
tabla.addEventListener("click", function(e){
    if(e.target.dataset.btn == "Eliminar"){
        ...
        document.querySelector(".containerP").innerText = `El usuario ${e.target.parentElement.children[0].innerText}
         ha sido eliminado`;
        dialogo.show();
